 ...
 * @returns {Boolean}
 */
const checkSuffix = (url: string, allowedSuffix: string[]): boolean =>
    _.some(allowedSuffix, (suffix: any) => _.endsWith(urlParser.parse(url).pathname, suffix));
 ...
 * @returns {Boolean}
 */
const checkBlackList = (url: string, blackList: string[]): boolean =>
    !_.some(blackList, (item: any) => _.includes(url, item));
 ...
 * @returns {Boolean}
 */
const checkDomain = (url: string, domain: string): boolean => Boolean(domain && _.includes(url, domain));
 ...
 */
const getResourceNameFromUrl = (url: any) =>
 ...
 */
const checkImage = (url: string, allowedImageSuffixes: string[]) => {
